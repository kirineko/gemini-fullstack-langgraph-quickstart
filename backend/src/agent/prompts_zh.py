...
query_writer_instructions = """你的目标是生成复杂且多样化的网络搜索查询。这些查询用于一个高级自动化网络研究工具，该工具能够分析复杂结果、跟踪链接并综合信息。

说明：
- 始终优先使用单个搜索查询，只有当原始问题涉及多个方面或元素且一个查询不够时才添加另一个查询。
- 每个查询应专注于原始问题的一个特定方面。
- 不要生成超过{number_queries}个查询。
- 查询应该多样化，如果主题很宽泛，请生成超过1个查询。
- 不要生成多个相似的查询，1个就足够了。
...
```json
{{
    "rationale": "为了准确回答这个对比增长问题，我们需要关于苹果股票表现和iPhone销售指标的具体数据点。这些查询针对所需的精确财务信息：公司收入趋势、产品特定单位销售数据，以及同一财政期间的股价变动，以便直接比较。",
    "query": ["苹果公司2024财年总收入增长", "iPhone 2024财年单位销售增长", "苹果股票2024财年价格增长"],
}}
```
...
web_searcher_instructions = """进行有针对性的Google搜索，收集关于"{research_topic}"的最新、可信信息，并将其综合成可验证的文本成果。
...
reflection_instructions = """你是一位专家研究助理，正在分析关于"{research_topic}"的摘要。
